...
class MarkdownGenerator:
    ...
    def __init__(self):
        ...
        self.required_file = open(base_json_browser_path + "required_fields.md",
                                  "w")
        self.required_file.write("# Required fields\n")
        self.required_file.write(
            "_This document contains a list of all required fields in the HCA metadata schema. For a full"
    ...
    def generateMarkdown(self, schemas, entity_type):
        ...
        for path in schemas:
            ...
            if (entity_type == "module" or entity_type == "core"):
                file.write("## " + schema["title"] + "<a name='" + schema[
                    "title"] + "'></a>\n")
                self.required_file.write(
                    "### " + schema["title"] + "<a name='" + schema[
                        "title"] + "'></a>\n")
            ...
            file.write("\n")
            file.write(
                "Location: " + path.replace(base_schema_path + "/", "") + "\n")
            file.write("\n")
            ...
            if "required" in schema:
                ...
                self.required_file.write(
                    "_There are no required properties in schema " + schema[
                        "title"] + "_\n")
            ...
            for property in schema["properties"]:
                ...
                if property not in boiler_plate:
                    ...
                    elif "items" in schema["properties"][property] and "$ref" in \
                    ...
                    file.write(property + " | " +
                               (schema["properties"][property][
                                    "description"] if "description" in
                                                      schema["properties"][
                                                          property] else "") + " | " +
                               (schema["properties"][property][
                                    "type"] if "type" in schema["properties"][
                                   property] else "") + " | " +
                               (
                                   "yes" if property in required else "no") + " | " +
                               link + " | " +
                               (schema["properties"][property][
                                    "user_friendly"] if "user_friendly" in
                                                        schema["properties"][
                                                            property] else "") + " | " +
                               (", ".join(schema["properties"][property][
                                              "enum"]) if "enum" in
                                                          schema["properties"][
                                                              property] else "") + " | " +
                               (str(schema["properties"][property][
                                        "example"]) if "example" in
                                                       schema["properties"][
                                                           property] else "") + "\n")
